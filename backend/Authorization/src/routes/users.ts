...
import { readableFields, User, userSchema, userUpdateSchema, forbiddenFieldsToFilter } from "../DB/Models/User";
...
users.get('/', authenticate, async (req, res) => {
    try {
        ...
        if (filterJson) {
            ...
            if (filter === undefined || FilterManagement.validateFilters<User>(filter, userSchema, undefined, forbiddenFieldsToFilter) !== true) {
