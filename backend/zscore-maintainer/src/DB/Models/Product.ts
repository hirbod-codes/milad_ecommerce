...
import { likeObjectId, localizedText, price, uniqueArrayTest } from "@monorepo/mongodb";
...
export const productSchema = object().required().strict(true).unknown(true).shape({
    ...
    weeklyViewZScore: number().required().strict(true).integer().nullable(),
    monthlyViewZScore: number().required().strict(true).integer().nullable(),
    yearlyViewZScore: number().required().strict(true).integer().nullable(),
